...
int socket_qrtr(int domain, int type, int protocol) {
	return socket(domain != AF_QIPCRTR ? domain : AF_MSM_IPC, type, protocol);
}
...
int getsockname_qrtr(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
	// getsockname_qrtr() on AF_MSM_IPC causes 'Unable to handle kernel NULL pointer dereference at virtual address 00000000'
	...
	if (translate_addr_qipcrtr2msmipc(dest_addr, &sami)) {
		...
		if (dest_addr_qrtr->sq_node == IPC_ROUTER_NID_LOCAL && dest_addr_qrtr->sq_port == QRTR_PORT_CTRL &&
			...
			if (bind(sockfd, (struct sockaddr*)&bind_addr, sizeof(bind_addr)) < 0) {
				// leave bind_qrtr()'s errno for caller
				return -1;
			}
		...
		return sendto(sockfd, buf, len, flags, (const struct sockaddr*)&sami, sizeof(sami));
	} else {
...
ssize_t recvfrom_qrtr(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen) {
	...
	sz = recvfrom(sockfd, buf, len, flags, (struct sockaddr*)&sami, &sami_len);
